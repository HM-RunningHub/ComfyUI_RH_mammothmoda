...
# def pad_input(tensor_unpad, indices_q, batch_size, seq_len):

#     dim = tensor_unpad.size(-1)
#     output = torch.zeros(batch_size, seq_len, dim, device=tensor_unpad.device, dtype=tensor_unpad.dtype)
    
#     for i in range(batch_size):
#         start_idx = indices_q[i]
#         length = min(seq_len, tensor_unpad.size(0) - start_idx)
#         output[i, :length] = tensor_unpad[start_idx:start_idx+length]
    
#     return output

# def unpad_input(output_tensor, padding_mask):
#     return output_tensor * padding_mask.unsqueeze(-1)
...
class AttnProcessorFlash2Varlen:
    ...
    def _upad_input(
        ...
        if query_length == kv_seq_len:
            ...
            # query_layer, indices_q, cu_seqlens_q, max_seqlen_in_batch_q = flash_attn_unpad(query_layer, attention_mask)
    ...
    def __call__(
        ...
        # hidden_states = flash_attn_pad(attn_output_unpad, indices_q, batch_size, sequence_length)
